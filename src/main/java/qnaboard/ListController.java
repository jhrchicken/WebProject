...
public class ListController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// 게시물 리스트
		List<QnaboardDTO> qnaboardLists = dao.selectListPage(map);
		dao.close();
		// 페이지 번호를 String으로 반환
		...
		request.setAttribute("qnaboardLists", qnaboardLists);
		request.setAttribute("map", map);
		// 포워드
		request.getRequestDispatcher("../Qnaboard/qnaboard.jsp").forward(request, response);
	}
}
