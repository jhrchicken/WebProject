...
public class BoardPage {
	...
	public static String pagingStr (int totalCount, int pageSize, int blockPage, int pageNum, String reqUrl) {
		...
		한 블럭 당 출력할 페이지의 번호의 개수는 현재 5로 설정됨
			현재 1페이지라 가정하면 (((1-1) / 5) * 5) + 1 = 1
			현재 4페이지라 가정하면 (((4-1) / 5) * 5) + 1 = 1
			=> 즉 1~5페이지의 경우에는 해당 블럭의 첫페이지는 1이다.
			현재 6페이지라 가정하면 (((6-1) / 5) * 5) + 1 = 6
			=> 즉 6~10페이지의 경우에는 해당 블럭의 첫페이지는 6이다. */
		int pageTemp = (((pageNum - 1) / blockPage) * blockPage + 1);
		/* pageTemp가 1이 아닐때만, 즉 첫번째 블럭이 아닌 경우에만 이전 블록 바로가기 링크를 화면에 출력한다. */
		if (pageTemp != 1) {
